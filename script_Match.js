...
function checkAnswer(userAnswer) {
  ...
  if (userAnswer === correctAnswer) {
    ...
    saveRoundAnswers(); // ✅ เพิ่มบันทึกข้อมูลตรงนี้!
...
function endGame() {
  ...
  localStorage.setItem("totalScore", totalScore + score); // Update totalScore for display
  localStorage.setItem("finalScore", score);

  // ✅ เก็บคำตอบล่าสุดไว้ใน `localStorage`
  localStorage.setItem("savedAnswers", JSON.stringify(currentRoundAnswers));

  // ✅ ตั้งค่าให้เกมรู้ว่าสิ้นสุดแล้ว
...
function updateProfileScore() {
  ...
  let matchScore = parseInt(
    localStorage.getItem(`score_${playerName}_match`) || 0
  );
  let puzzleScore = parseInt(
    localStorage.getItem(`score_${playerName}_puzzle`) || 0
  );
...
function saveMatchScore(finalScore) {
  let playerName = localStorage.getItem("playerName") || "Guest";
  let currentMatchScore = parseInt(
    localStorage.getItem(`score_${playerName}_match`) || 0
  );
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (urlParams.get("showGameComplete") === "true") {
    console.log("🔹 เปิด Game Complete Popup");
    document.getElementById("game-complete-popup").style.display = "flex";
  }
});
